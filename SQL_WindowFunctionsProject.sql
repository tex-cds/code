###############################################################################################
-- Coursera Guided Project:  SQL Window Functions
-- Topics Covered:           ROW_NUMBER, PARTITION BY, LEAD/LAG, CTEs, FIRST_VALUE, NTH_VALUE
###############################################################################################
...
-- CAUTION: ORDERING OUTSIDE OF WINDOW FUNCTION SUPERCEDES ORDERING INSIDE WF
...
-- Exercise 4.2: Order by the hire_date
...
-- Exercise 5.4: Number each customer by their customer segment
...
-- Exercise 6.4: Retrieve all employees first name, department, salary
...
-- Exercise 6.5: Retrieve all employees first name, department, salary
...
-- ANSWER: it returns the next 2 closest salaries to the salary value
SELECT first_name, department, salary,
...
FIRST_VALUE(hire_date) OVER(ORDER BY hire_date ASC) AS first_emp_date
FROM employees
...
-- Exercise 7.3: Partition by department
SELECT first_name, last_name, department, hire_date,
FIRST_VALUE(hire_date) OVER (PARTITION BY department ORDER BY hire_date) AS first_emp_date
FROM employees;

-- Exercise 7.4: Find the difference between the hire date of the 
...
-- Order by the first_name in ascending order
SELECT  first_name, email, department, salary,
NTH_VALUE(salary, 5) OVER(PARTITION BY department ORDER BY first_name ASC) AS fifth_salary
FROM employees;